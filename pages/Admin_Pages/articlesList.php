...
<body>
    ...
    <div id="main">
        ...
        <div class="head">
            <div class="sec-name">
                <p class="nav">Makaleler Listesi</p>
            </div>
            <div class="sec-name">
                <input class="nav-sch-add"></input>
            </div>
            <div class="sec-name">
                <button class="nav-button-add"><a href="../Addition_Pages/addArticlePanel.php">Makale Ekle</a></button>
            </div>
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '<p class="head-1">' . $row["articles_name"] . '</p>';
                echo '<div class="up-del">';
                echo '<i class="fa-solid fa-ellipsis-vertical fa-lg list-edit" onclick="toggleMenu(' . $row["articles_id"] . ')"></i>';
                echo '<ul class="list-unstyled" id="menu-' . $row["articles_id"] . '" style="display: none;">';
                echo '<li><a href="../Updates_Pages/updateArticlePanal.php?article_id=' . $row["articles_id"] . '">Güncelle</a></li>';
                echo '<li class="but-delete" id="delete-but-article" data-article-id="' . $row['articles_id'] . '">Sil</li>';
                echo '</ul>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
