...
<body>
    ...
    <div id="main">
        ...
        <div class="head">
            <div class="sec-name">
                <p class="nav">Olaylar List</p>
            </div>
            <div class="sec-name">
                <input class="nav-sch-add"></input>
            </div>
            <div class="sec-name">
                <button class="nav-button-add"><a href="../Addition_Pages/addEventPanel.php">Olay Ekle</a></button>
            </div>
        ...
        $sql = "SELECT * FROM events"; // Sıralama olmadan tüm makaleleri seç
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '<div class="box">';
                echo '<img src="../../imgs/cover-box-ava.jpg">';
                echo '<div class="bot-bar">';
                echo '<p class="head-1">' . $row["event_name"] . '</p>';
                echo '<div class="up-del">';
                echo '<i class="fa-solid fa-ellipsis-vertical fa-lg list-edit" onclick="toggleMenu(' . $row["event_id"] . ')"></i>';
                echo '<ul class="list-unstyled" id="menu-' . $row["event_id"] . '" style="display: none;">';
                echo '<li><a href="../Updates_Pages/updateEventPanel.php?event_id=' . $row["event_id"] . '">Güncelle</a></li>';
                echo '<li class="but-delete" id="delete-but-event" data-event-id="' . $row['event_id'] . '">Sil</li>';
                echo '</ul>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
